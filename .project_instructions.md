...
## v4-fix12a/b/c: Stand Marker Zoom System (Final Architecture)
...
- **Offset (v4-fix12c corrected sign)**: `offset: [0, yOffset]` (positive y = DOWN). `yOffset = HITBOX_SIZE/2 - discFromTop`.
- **Phase 1 (motion)**: ONLY `wrapper.style.transform = scale(...)`. No SVG rebuild, no innerHTML, no opacity changes.
- **Phase 2 (settle, 200ms)**: ONE SVG rebuild if zoom delta > 0.3. Updates wrapper transform-origin, visual size, marker offset.
- **`cameraMovingRef`**: Gates stand toggle — if moving, snap without transitions. `setOffset()` cast as `any` (exists at runtime, missing from TS types).
- **Diagnostics**: `[STAND-DIAG] id=N rebuiltAfterSettle=true transformOrigin=... offset=...`
