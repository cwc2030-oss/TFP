...
## v4-fix20: Fixed-Size Stand Markers (replaces v4-fix12/13/18/19)
- **Philosophy**: Stands should feel "planted in the land", not floating UI. Fixed pixel size, no zoom scaling, no transform stack.
- **Architecture**: FLAT — `el` (Mapbox positioning) → `.stand-visual` (SVG) + `.stand-badge` + tooltip. No scale wrapper. Mapbox owns `el.style.transform` entirely. We only touch `el.style.opacity`.
- **Fixed sizes**: #1=52px, #2=46px, #3=40px — no zoom-responsive scaling, no `getMarkerScale()`, no `lastSVGRebuildZoom`.
- **Anchor**: `anchor: 'bottom'` — pole base sits on the geo coordinate. No offset math.
- **Hover**: Filter glow only (`brightness + drop-shadow`), no scale transforms.
- **Reveal**: Opacity fade-in only (300ms, 40ms stagger), no scale animation.
- **Visibility toggle**: Opacity only, no wrapper transforms.
- **Camera sync**: Minimal — `cameraMovingRef` tracks motion state for reveal gating only. No scaling during motion, no settle-time transforms.
- **Result**: Zero positional drift by design — Mapbox handles all positioning, we never compete with its transforms.
