...
## v4-fix12b: Flicker-Free Marker Zoom
- **Problem**: Markers flickered during scroll/zoom due to DOM mutations happening mid-motion.
- **Root causes found & fixed**:
  1. Entry animation `transition` lingered on elements — camera-sync `scale()` changes were CSS-animated (lagging) instead of instant. **Fix**: Strip `transition` on first move frame; single cleanup timeout after last marker finishes entry.
  2. `will-change: transform` was toggled ON during first move frame, forcing mid-motion layer promotion repaint. **Fix**: Set `will-change` at marker creation time, never toggle during motion.
  3. Per-frame `console.error` logging inside rAF dropped frames. **Fix**: Removed all per-frame logging; only log on settle.
  4. Stand toggle effect set `transition`/`opacity`/`transform` regardless of camera state. **Fix**: `cameraMovingRef` gates — if camera moving, snap without transitions.
- **`cameraMovingRef`**: Component-level ref (`useRef(false)`), set `true` on first `move` event, set `false` when `rebuildMarkersAtZoom` runs after 200ms settle debounce.
- **Phase 1 invariant**: During motion, ONLY `el.style.transform` is touched. No innerHTML, no opacity, no visibility, no transition, no willChange changes.
- **Phase 2**: After 200ms settle, ONE rebuild pass (if zoom delta > 0.3), then stop.
- **Stand toggle**: Reads `cameraMovingRef.current` — if moving, snaps opacity/pointerEvents without transition. If settled, uses staggered cinematic transitions with cleanup timers.
