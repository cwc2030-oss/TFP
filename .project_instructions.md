...
## Territory Oklahoma Silent Failure Diagnosis (April 2026)
- **Root cause**: `/api/parcels/lookup/route.ts` has a **hardcoded app-side bounds gate** at lines ~110-138 that rejects any lat/lng not inside KS (36.99-40.00 / -102.05 to -94.59) or MO (35.99-40.61 / -95.77 to -89.10). Oklahoma (~33.62-37.00 lat, -103 to -94.43 lng) falls outside both → returns `{found: false, error: "Location outside KS/MO region..."}` with HTTP 200.
- **Regrid coverage is NOT the problem**: Regrid's public docs confirm coverage for ALL 77 Oklahoma counties with 335,468 parcels in Oklahoma County alone, last refreshed Feb 10 2026. Same Parcel API tier we already use.
- **Fix to enable Oklahoma**: Single-line change — add `inOK` check to region gate: `const inOK = lat >= 33.6 && lat <= 37.0 && lng >= -103.0 && lng <= -94.4` and change `if (!inKS && !inMO)` to `if (!inKS && !inMO && !inOK)`. (Not yet applied — awaiting user confirmation.)
- **Enhanced diagnostics added** (this commit):
  - **Client**: `attemptTerritoryLookup` in Territory Mode path of `handleParcelPick` (`app/intel/page.tsx` ~8107-8175) now reads lookup response as text first, safe JSON parses, and dumps `[TERRITORY-REGRID-DIAG]` console.error with `attempt`, `clickLat/Lng`, `httpStatus`, `errorCode`, `fullResponseBody`, `debugInfo` on any failure.
  - **Toast UX**: 3-branch user-facing toast.error with 8s duration:
    1. If error matches `outside KS/MO|region` → "That location is outside our current parcel coverage area. Please click within Kansas, Missouri, or an adjacent supported state."
    2. If `rate limit` → rate-limit message
    3. Generic → "No parcel boundary found here — Regrid coverage may be limited in this county. Try clicking further into the field interior."
  - **Server**: `/api/parcels/lookup/route.ts` region gate now logs `[PARCEL LOOKUP] Region gate blocked request` with lat/lng/likelyState ('OK' or 'OUT_OF_REGION') and returns `likelyState` in response.
  - **Server**: Regrid fetch block (~204-271) reads body as text first → safe JSON parse → logs `[PARCEL LOOKUP]` errors with full body on non-JSON, HTTP error, status=error, and 0-results cases. Response now includes `regrid` field with upstream body on failures.
- **Log prefixes**: `[TERRITORY-REGRID-DIAG]` = client-side territory lookup failure; `[PARCEL LOOKUP]` = server-side lookup route. The `likelyState` response field labels bounds rejections ('OK' | 'OUT_OF_REGION').
- **Scope**: Only the Territory Mode click path in `handleParcelPick` was modified. The single-parcel path (line ~8203) and QA path (line ~7784) still use the original silent-return pattern — leave untouched unless user requests broader changes.
