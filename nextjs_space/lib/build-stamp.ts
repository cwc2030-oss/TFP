...
//
// r36 — ON-MAP ACTION BAR + TERRAIN BRAIN(TM) REPORT BRANDING (Claude patch
//      SENDTOABACUS.patch, 2 commits). (1) intel/page.tsx: floating bottom-center
//      action bar wiring existing Save/Claim/List handlers, one tap from the map;
//      gated to Pro on a single loaded parcel, hidden in territory/Clean Map/export.
//      (2) Report + dashboard branding: report header (every page) + social preview
//      now read 'Powered by Terrain Brain(TM)'; dashboard 'Your Reports' -> 'Your
//      Terrain Brain(TM) Reports'. App/UI + report-copy only, no engine change.
