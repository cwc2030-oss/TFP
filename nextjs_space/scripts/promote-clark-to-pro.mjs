// One-time script: promote Clark (cwc2030@gmail.com) to Pro tier directly in DB.
// Owner/admin should always reflect active Pro on the live site.
...
async function main() {
  ...
  const after = await prisma.user.update({
    ...
    data: {
      ...
      role: before.role === 'admin' ? 'admin' : 'admin', // ensure admin role too
    },
    select: {
      id: true,
      email: true,
  ...
  console.log('[promote] ✅ Clark is now Pro tier (perpetual).');
}
