...
function buildParcelPath(coordinates: number[][][] | null): string {
  ...
  const pathPoints = ring.filter((_, i) => i % step === 0 || i === ring.length - 1).map(coord => `${coord[1]},${coord[0]}`).join("|");
...
export async function POST(request: NextRequest) {
  try {
    ...
    const terrainMap = await fetchGoogleMapImage(order.parcelLat, order.parcelLng, "terrain", optimalZoom, parcelData.coordinates);
