...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      const modalResponse = await fetch(RIDGE_API_URL, {
        ...
        body: JSON.stringify({
          ...
          options: {
            ...
            min_prominence_ft: 15,     // Relaxed for Ozark terrain (was 25)
            min_length_m: 100,         // Relaxed — shorter ridges are valid in hilly terrain (was 150)
    ...
    // Preserve unfiltered ridges for saddle proximity checks — saddles sit on Modal-detected
    // ridges that may fail spine quality (curvature/length) but are still valid anchor references.
    const unfilteredRidgesPrimary = ridgeData.ridges_primary;
    const unfilteredRidgesSecondary = ridgeData.ridges_secondary;
    ...
    // Use UNFILTERED ridges for saddle proximity — saddles generated by Modal sit along ridges
    // that may fail curvature/length checks but still mark valid terrain saddle points.
    ...
    const { filtered: filteredSaddles, debug: saddleDebug } = filterSaddlesByQuality(
      ...
      unfilteredRidgesPrimary,
      unfilteredRidgesSecondary,
    );
