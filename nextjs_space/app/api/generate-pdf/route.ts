...
export async function POST(request: NextRequest) {
  try {
    ...
    if (order.productType === 'hunt_report') {
      ...
      const displayAcreage = terrainPayload.acreage ? Math.round(terrainPayload.acreage) + ' Acres' : '';
      const combinedHtml = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>TerraFirma Hunt Package — ${order.parcelAddress || 'Property'}</title>
<style>
  body { margin: 0; font-family: Georgia, serif; background: #0a1a10; }
  .cover { background: #1a3a2a; color: white; padding: 60px 80px; min-height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; page-break-after: always; }
  .cover h1 { font-size: 48px; color: #c9a84c; margin-bottom: 16px; letter-spacing: 3px; }
  .cover h2 { font-size: 24px; color: white; margin-bottom: 8px; }
  .cover p { font-size: 14px; color: #888; margin-top: 32px; }
  .gold-bar { height: 3px; background: linear-gradient(90deg, #c9a84c, #f0d080, #c9a84c); width: 300px; margin: 24px auto; }
  .section-label { font-size: 12px; letter-spacing: 4px; color: #52b788; text-transform: uppercase; margin-bottom: 8px; }
  .divider { border: none; border-top: 1px solid #2d6a4f; margin: 40px 0; }
  .report-section { page-break-before: always; }
  .land-pdf-section { page-break-before: always; background: #0d2018; padding: 60px; text-align: center; min-height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; }
  .land-pdf-section h2 { color: #c9a84c; font-size: 28px; margin-bottom: 16px; }
  .land-pdf-section p { color: #888; font-size: 14px; margin-bottom: 32px; }
  .land-pdf-btn { display: inline-block; background: #2d6a4f; color: #c9a84c; padding: 16px 40px; font-size: 16px; font-weight: bold; border-radius: 8px; text-decoration: none; letter-spacing: 1px; border: 1px solid #52b788; }
  .land-pdf-btn:hover { background: #3a7d5e; }
</style>
</head>
<body>
...
<!-- COVER PAGE -->
<div class="cover">
  <div class="section-label">Terra Firma Partners</div>
  <h1>HUNT PACKAGE</h1>
  <div class="gold-bar"></div>
  <h2>${order.parcelAddress || 'Property Analysis'}</h2>
  <p style="color:#c9a84c;font-size:16px">${displayAcreage}</p>
  <hr class="divider" style="width:300px;border-color:#2d6a4f">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-top:32px;text-align:left">
    ...
    <div style="background:#0d2018;padding:24px;border:1px solid #2d6a4f;border-radius:8px">
      <div style="color:#c9a84c;font-size:11px;letter-spacing:2px;margin-bottom:8px">INCLUDED</div>
      <div style="color:white;font-size:15px;font-weight:bold;margin-bottom:4px">Land Intelligence Report</div>
      <div style="color:#888;font-size:12px">Property overview, soil analysis, flood risk, market data, county hunting seasons</div>
    </div>
  </div>
  <p style="margin-top:48px;color:#555;font-size:12px">Generated by terrafirma.partners — Terrain Intelligence for Serious Hunters</p>
</div>

<!-- HUNT INTELLIGENCE REPORT -->
<div class="report-section">
${huntHtml.replace(/<!DOCTYPE html>/i, '').replace(/<html[^>]*>/i, '').replace(/<\/html>/i, '').replace(/<head>[\s\S]*?<\/head>/i, '')}
</div>

<!-- LAND INTELLIGENCE PDF — embedded download -->
<div class="land-pdf-section">
  <div class="section-label">Bonus Report</div>
  <h2>Land Intelligence Report</h2>
  <p>Your full property analysis PDF is embedded below.<br>Click the button to save it separately.</p>
  <a class="land-pdf-btn" download="TFP-Land-Report-${propertyName}.pdf" href="data:application/pdf;base64,${landPdfBase64}">
    &#128196; Download Land Intelligence PDF
  </a>
  <p style="margin-top:24px;color:#555;font-size:11px">Property overview &bull; Soil analysis &bull; Flood risk &bull; Market data &bull; Hunting seasons</p>
</div>

</body>
</html>`;

      const base64 = Buffer.from(combinedHtml).toString('base64');

      return NextResponse.json({
        pdf: base64,
        filename: `TFP-Hunt-Package-${propertyName}.html`,
