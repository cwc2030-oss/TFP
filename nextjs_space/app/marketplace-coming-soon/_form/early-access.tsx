...
export default function EarlyAccess() {
  ...
  if (submitted && side) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-2xl font-bold text-stone-900">You&apos;re on the list.</h3>
        <p className="mt-3 text-stone-600 max-w-md mx-auto">{PATHS[side].done}</p>
      </div>
    );
  }
    ...
    return (
      <div>
        <div className="text-center">
          <h3 className="text-xl font-bold text-stone-900">Get Early Access</h3>
          <p className="mt-1 text-stone-500">Choose your path below</p>
        </div>
          ...
          {(Object.keys(PATHS) as Side[]).map((key) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setSide(key)}
                className="text-left rounded-xl border border-stone-200 bg-white p-5 hover:border-emerald-500 hover:shadow-sm transition-colors group"
              >
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-emerald-50 text-emerald-700 mb-3 group-hover:bg-emerald-100">
                  <Icon className="w-5 h-5" />
                </span>
                <p className="font-semibold text-stone-900">{p.title}</p>
                <p className="text-sm text-stone-500 mt-0.5">{p.blurb}</p>
              </button>
            );
          })}
        </div>
  ...
  return (
    <div>
      ...
      <p className="text-stone-500 mt-0.5">{p.blurb}</p>
      ...
      <form onSubmit={handleSubmit} className="mt-5 space-y-3">
        <input
          ...
          className="w-full rounded-lg border border-stone-300 px-4 py-3 text-stone-900 placeholder-stone-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none"
        />
        <input
          name="email"
          type="email"
        ...
        <select
          name="state"
          defaultValue=""
          className="w-full rounded-lg border border-stone-300 px-4 py-3 text-stone-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none"
        >
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-700 hover:bg-emerald-600 disabled:opacity-60 text-white font-semibold px-6 py-3 transition-colors"
        >
